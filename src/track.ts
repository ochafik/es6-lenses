...
interface AccessHook {
  logAccess(target: any, targetPath: PropertyKey[], p: PropertyKey);
}

function wrap<T>(object: T, path: PropertyKey[], hook: AccessHook): T {
  if (object == null ||
  ...
  if (object instanceof Function) {
    let wrappedFunction = function(...args: any[]) {
      return wrap((object as Function).apply(rootObject, args), null);
    };
    wrappedFunction[getTargetSym] = object;
    return wrappedFunction as T;
  }
  return new Proxy<Proxied<T>>(new Proxied<T>(object, path, hook), sandboxProxyHandler) as T;
}
...
const sandboxProxyHandler = new SandboxProxyHandler();

const root = wrap(rootObject, {
  logAccess: function(target: any, targetPath: PropertyKey[], p: PropertyKey) {
    console.log(`ACCESSING: ${targetPath.join('.')}.${p}`);
  }
});
